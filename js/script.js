...
{topic:"hamming", q:"Um código de Hamming com 4 bits de dados gera uma palavra codificada de quantos bits?", opts:["4","7","8","11"], correct:1, exp:"É o Hamming (7,4): 4 bits de dados + 3 bits de paridade = 7 bits totais.", calc:true},
...
{topic:"hamming", q:"Se os bits de paridade calculados forem p8=0, p4=1, p2=0, p1=1, em qual posição decimal está o bit com erro?", opts:["Posição 3","Posição 5","Posição 9","Posição 10"], correct:1, exp:"Juntando p8 p4 p2 p1 = 0101 em binário, que corresponde a 5 em decimal — o bit 5 está com erro.", calc:true},
...
{topic:"ondas", q:"Se uma onda tem frequência de 20 Hz e comprimento de onda de 18 cm, qual é a sua velocidade?", opts:["180 cm/s","360 cm/s","0,9 cm/s","38 cm/s"], correct:1, exp:"v = λ · f = 18 cm × 20 Hz = 360 cm/s, exatamente como no exemplo do material.", calc:true},
...
{topic:"paridade", q:"Usando paridade ímpar (bit de paridade incluso no total), qual das sequências de 8 bits a seguir apresenta erro de transmissão: 11010101, 01101110 ou 10010011?", opts:["11010101","01101110","10010011","Nenhuma apresenta erro"], correct:2, exp:"Contando os bits 1 de cada sequência: 11010101 e 01101110 têm 5 uns (ímpar, OK); 10010011 tem 4 uns (par), o que viola a paridade ímpar esperada — logo apresenta erro.", calc:true},
{topic:"paridade", q:"Completando a palavra de dados 1110001 com um bit de paridade par no final, qual o bit adicionado e a palavra final?", opts:["Bit 1, formando 11100011","Bit 0, formando 11100010","Bit 1, formando 11100010","Não é possível determinar sem mais informações"], correct:1, exp:"A palavra 1110001 já tem 4 bits 1 (quantidade par), então o bit de paridade par adicionado é 0, resultando em 11100010.", calc:true},
...
{topic:"propagacao", q:"Um sinal precisa ser transmitido por 1000 km através de fibra óptica, com velocidade de propagação de 200.000 km/s. Qual é o tempo de propagação?", opts:["2 ms","5 ms","10 ms","50 ms"], correct:1, exp:"Tempo de propagação = distância / velocidade = 1000 km / 200.000 km/s = 0,005 s = 5 ms.", calc:true},
{topic:"propagacao", q:"Um sinal percorre 500 km em um meio com velocidade de propagação de 250.000 km/s. Qual é o tempo de propagação?", opts:["0,5 ms","1 ms","2 ms","4 ms"], correct:2, exp:"Tempo de propagação = 500 km / 250.000 km/s = 0,002 s = 2 ms.", calc:true},
{topic:"propagacao", q:"Um sinal precisa ser transmitido por 1200 km através de fibra óptica, a 200.000 km/s. Qual é o tempo de propagação?", opts:["3 ms","6 ms","12 ms","60 ms"], correct:1, exp:"Tempo de propagação = 1200 km / 200.000 km/s = 0,006 s = 6 ms.", calc:true},
{topic:"propagacao", q:"Um pacote viaja 3000 km a 200.000 km/s entre A e B. O processamento no servidor B leva 2 ms e o enfileiramento nos dispositivos intermediários leva 1 ms em cada direção. Qual é o RTT total?", opts:["15 ms","30 ms","34 ms","40 ms"], correct:2, exp:"Tempo de propagação de ida = 3000/200.000 = 15 ms; o RTT considera ida e volta (2×15=30 ms) mais processamento (2 ms) mais enfileiramento nas duas direções (1+1=2 ms). Total = 30+2+2 = 34 ms.", calc:true},
...
{topic:"propagacao", q:"Um pacote de 8000 bits é enviado por um enlace com taxa de transmissão de 2 Mbps. Qual é o tempo de transmissão do pacote (desconsiderando a propagação)?", opts:["2 ms","4 ms","8 ms","16 ms"], correct:1, exp:"Tempo de transmissão = tamanho do pacote / taxa de transmissão = 8000 bits / 2.000.000 bps = 0,004 s = 4 ms. É um conceito diferente do tempo de propagação.", calc:true},
{topic:"propagacao", q:"Um arquivo de 1.000.000 bits precisa ser enviado por um link de 500 kbps. Quanto tempo leva a transmissão, desconsiderando a propagação?", opts:["0,5 s","1 s","2 s","4 s"], correct:2, exp:"Tempo de transmissão = 1.000.000 bits / 500.000 bps = 2 s.", calc:true},
{topic:"propagacao", q:"Um enlace tem 2000 km de extensão, velocidade de propagação de 200.000 km/s e taxa de transmissão de 1 Mbps. Um pacote de 100.000 bits é enviado. Qual o tempo total (transmissão + propagação) até o pacote chegar ao destino?", opts:["10 ms","100 ms","110 ms","210 ms"], correct:2, exp:"Tempo de transmissão = 100.000 bits / 1.000.000 bps = 100 ms. Tempo de propagação = 2000 km / 200.000 km/s = 10 ms. Total = 100 + 10 = 110 ms.", calc:true},

{topic:"ondas", q:"Uma onda sonora tem frequência de 500 Hz e se propaga a 340 m/s. Qual é o seu comprimento de onda?", opts:["0,68 m","1,47 m","170 m","840 m"], correct:0, exp:"λ = v / f = 340 / 500 = 0,68 m.", calc:true},

{topic:"crc", q:"Usando o polinômio gerador CRC-4 x⁴+x+1 (equivalente a 10011) para os dados 10110011, qual é o CRC de 4 bits obtido?", opts:["0100","1110","0001","1011"], correct:0, exp:"Anexando 4 zeros (101100110000) e dividindo por 10011 com XOR, o resto é 0100 — esse é o CRC, formando a mensagem transmitida 101100110100.", calc:true},
{topic:"crc", q:"A mensagem 1101011011 foi transmitida com CRC (gerador 10011), resultando em 11010110111110. Se um único bit dessa mensagem for corrompido durante a transmissão e o receptor dividir tudo de novo pelo mesmo gerador, o resultado será:", opts:["Resto zero, como se não houvesse erro","Resto diferente de zero, indicando erro","O CRC original (1110) novamente","Impossível saber sem repetir a transmissão"], correct:1, exp:"Ao alterar um bit da mensagem transmitida (por exemplo, o 4º bit) e dividir por 10011, o resto passa a ser 0111 (diferente de zero) — a divisão só dá resto zero quando não há erro.", calc:true},

{topic:"paridade", q:"A mensagem 1010110 usa paridade par. Se o último bit fosse alterado para 1 (ficando 1010111), qual seria o novo bit de paridade?", opts:["0","1","O bit de paridade não mudaria","Não é possível calcular"], correct:1, exp:"1010111 tem 5 bits em 1 (ímpar). Para paridade par, o total (dados + paridade) precisa ser par, então o bit de paridade deve ser 1 (5+1=6, par).", calc:true},
{topic:"paridade", q:"Um transmissor usa paridade ímpar para enviar a mensagem 1110001 (transmitida como 11100011). Se o receptor recebe 11100010 em vez disso, o que a verificação de paridade indica?", opts:["Nenhum erro, os dados chegaram corretos","Erro detectado, pois a quantidade de bits 1 é par (deveria ser ímpar)","Erro detectado, mas apenas no bit de paridade, nunca nos dados","Impossível determinar sem retransmissão"], correct:1, exp:"11100010 tem 4 bits em 1 (par), o que contraria a paridade ímpar esperada — um erro é detectado (embora não se saiba qual bit exatamente, só a paridade simples).", calc:true},

{topic:"hamming", q:"Uma mensagem com 7 bits de dados (1001011) precisa ser protegida com código de Hamming. Quantos bits de paridade são necessários e qual o tamanho total da palavra codificada?", opts:["3 bits de paridade, 10 bits totais","4 bits de paridade, 11 bits totais","4 bits de paridade, 12 bits totais","5 bits de paridade, 12 bits totais"], correct:1, exp:"Com m=7 bits de dados, é preciso o menor r tal que 2^r ≥ m+r+1. Para r=4: 2^4=16 ≥ 7+4+1=12, então são 4 bits de paridade e 7+4=11 bits totais.", calc:true},
{topic:"hamming", q:"A mensagem 0110011 foi transmitida usando Hamming(7,4). Ao recalcular os bits de paridade no receptor, o resultado (síndrome) é zero. Isso significa que:", opts:["Há um erro no bit de posição 3","Há um erro no bit de posição 5","A mensagem chegou sem erros","É impossível verificar com síndrome zero"], correct:2, exp:"Quando todos os bits de paridade recalculados batem (síndrome = 0), a palavra recebida é um código Hamming válido — não há erro detectado.", calc:true},
...
/* ---------- CÁLCULOS ---------- */
let calcState = { order:[], idx:0, answers:{} };
function updateStartCalcButton(){
  const n = QUESTIONS.filter(q=>q.calc).length;
  document.getElementById('start-calc').textContent = `Começar (${n} questões)`;
}
document.getElementById('start-calc').addEventListener('click', ()=>{
  const pool = shuffle(QUESTIONS.map((q,i)=>({...q,gid:i})).filter(q=>q.calc));
  Object.assign(calcState, { order: pool, idx:0, answers:{} });
  document.getElementById('calc-intro').style.display='none';
  document.getElementById('calc-quiz-area').style.display='block';
  document.getElementById('calc-result').style.display='none';
  renderQuizQuestion(calcState, 'calc');
});
initQuizNav(calcState, 'calc', ()=> finishQuiz(calcState, 'calc', ()=>{
  document.getElementById('calc-intro').style.display='block';
}));
